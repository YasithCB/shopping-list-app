...
class _GroceryListState extends State<GroceryList> {
  ...
  Future<List<GroceryItem>> _loadItem() async {
    final url = Uri.https(
        'flutter-check-db1-default-rtdb.firebaseio.com', 'shopping-list.json');
  ...
  void _removeItem(GroceryItem item) async {
    ...
    final url = Uri.https(
      'flutter-check-db1-default-rtdb.firebaseio.com',
      'shopping-list/${item.id}.json',
    );
    final resp = await http.delete(url);

    if (resp.statusCode >= 400) {
      // show error msg in here
      setState(() {
      ...
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data!.isEmpty) {
            return const Center(
              child: Text('No grocery items available!'),
            );
          }
